...
.edit-btn:hover {
    ...
    <main>
                ...
                <form method="get" action="${pageContext.request.contextPath}/userlist">
                    <label>
                        ...
                        <select name="genderFilter">
                            ...
                            <option value="0" <c:if test="${genderFilter == '0'}">selected</c:if>>Male</option>
                            <option value="1" <c:if test="${genderFilter == '1'}">selected</c:if>>Female</option>
                        </select>
                    </label>
        ...
        <a href="${pageContext.request.contextPath}/useradd" class="add-btn">➕ Add New User</a>
        
        <div class="line-limit-control">
            <label>
                Line number displayed:
                <input type="number" id="rowsPerPage" value="10" min="1" />
            </label>
        </div>               
        ...
        <div class="table-container">          
                ...
                <script>
                    ...
                    // Giới hạn số dòng hiển thị trong bảng theo input
                    const rowsInput = document.getElementById("rowsPerPage");     // Lấy input số dòng (id="rowsPerPage")
                    if (rowsInput) {                                              // Nếu tồn tại input này
                        rowsInput.addEventListener("input", () => {               // Khi người dùng thay đổi giá trị
                            const limit = parseInt(rowsInput.value);              // Lấy số dòng tối đa cần hiển thị
                            const rows = document.querySelectorAll("tbody tr");   // Lấy tất cả các dòng trong bảng
                            rows.forEach((row, index) => {                        // Duyệt từng dòng
                                row.style.display = index < limit ? "" : "none";  // Nếu chỉ số dòng < giới hạn → hiển thị, ngược lại ẩn
                            });
                        });

                        window.addEventListener("DOMContentLoaded", () => {       // Khi trang tải xong
                            rowsInput.dispatchEvent(new Event("input"));          // Tự động áp dụng giới hạn dòng đã chọn
                        });
                    }
